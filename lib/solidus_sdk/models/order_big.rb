  ...
  class OrderBig
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SolidusSdk::OrderBig` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SolidusSdk::OrderBig`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'ship_address')
        self.ship_address = attributes[:'ship_address']
      end
    ...
    def hash
      [additional_tax_total, adjustment_total, adjustments, bill_address, canceler_id, channel, checkout_steps, completed_at, covered_by_store_credit, created_at, credit_cards, currency, display_additional_tax_total, display_included_tax_total, display_item_total, display_order_total_after_store_credit, display_ship_total, display_store_credit_remaining_after_capture, display_tax_total, display_total, display_total_applicable_store_credit, display_total_available_store_credit, email, id, included_tax_total, item_total, line_items, number, order_total_after_store_credit, payment_methods, payment_state, payment_total, payments, permissions, ship_address, ship_total, shipment_state, shipments, special_instructions, state, tax_total, token, total, total_applicable_store_credit, total_quantity, updated_at, user_id].hash
    end
